...
import static uk.co.optimisticpanda.jarcompare.util.TestAssertions.assertThat;
import static uk.co.optimisticpanda.jarcompare.util.TestUtils.getFile;
...
public class JarContentsTest {
	...
	public void checkNumberOfClassesIsSameAsNumberOfCollectedClassesWithNestedClasses(){
		...
		Long totalClassCount = JarReader.<Long>entriesOf(VERSION_0_0_1_FILE).map(s -> s.count());
		
		Long collectedClassCount = JarContents.load(VERSION_0_0_1_FILE).stream()
